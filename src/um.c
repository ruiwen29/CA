...
void init(char *filename)
{
  ...
  fseek(fp, 0, SEEK_END); //pointer vers la fin du ficher
  size = ftell(fp);
  tab_plateaux = calloc(taille_collection, sizeof(*tab_plateaux)); //allocation pour les tableaux correspondant la taille
...
int runtime()
{
  ...
  while (1)
  {
    ...
    case 6: /** Not-And **/
      registre[A] = ~((unsigned) registre[B] & (unsigned)registre[C]);
      //printf(" regA %d = ~(regB %d & regC %d ) \n",registre[A],registre[B],registre[C]);
      break;
